...
# 無免疫者
def suspectiable_delta(t, diff = 0):
  i = t - diff
  if i < 0:
    i = 0
...
# 国内に発生したウィルス所持者の初期値
infected_count = inflected_first
# 無感染者の割合
...
for i in list(range(width)):
  ...
  suspectiable_count += -1 * suspectiable_delta_list[i]
...
for i in list(range(width)):
  plot_suspectiable_list[int(i/m)] += int(suspectiable_delta_list[i])
